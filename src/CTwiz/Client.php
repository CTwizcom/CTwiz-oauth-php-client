...
class Client
{
    ...
    const AUTH_END_POINT = 'http://ctwiz.com/account/authorize';
    const TOKEN_END_POINT = 'http://ctwiz.com/oauth2/access_token';
    const RESOURCE_END_POINT = 'http://api.ctwiz.com/v1/';
     ...
     * @param sting $redirectUrl where the user should be redirected to.
     ...
     * @return void
     */
    public function __construct($client_id, $client_secret, $redirectUrl)
    {
        ...
        $this->client_auth = self::AUTH_TYPE_URI;
    }
    ...
    public function getAuthenticationUrl($scope = "")
    {
        ...
        return self::AUTH_END_POINT . '?' . http_build_query($parameters, null, '&');
    }


    public function getAccessToken($code = null)
    {
        return $this->fetchAccessToken("authorization_code", array(
            "code" => $code,
            "redirect_uri" => $this->redirectUrl
        ));
    }
    ...
    {
        return $this->fetchAccessToken("refresh_token", array(
     ...
     */
    private function fetchAccessToken($grant_type, array $parameters)
    {
        if (!$grant_type) {
        ...
        $result = $this->executeRequest(SELF::TOKEN_END_POINT, $parameters, 'POST', $http_headers, self::HTTP_FORM_CONTENT_TYPE_APPLICATION);
        if (empty($result['error'])){
            return $result['result'];
        }
        else {
            throw new InvalidArgumentException($result['result']['message']);
        }
    }
    ...
    public function setAccessToken($info)
    {
        ...
        $this->access_token_type = $info['token_type'];
    }
    ...
    public function fetch($protected_resource_url, $parameters = array(), $http_method = self::HTTP_METHOD_GET, array $http_headers = array(), $form_content_type = self::HTTP_FORM_CONTENT_TYPE_MULTIPART)
    {
        ...
        $result = $this->executeRequest(self::RESOURCE_END_POINT . $protected_resource_url, $parameters, $http_method, $http_headers, $form_content_type);
        if (!empty($result['result']['result'])) {
            return $result['result']['result'];
        } else {
            throw new InvalidArgumentException(isset($result['result']['message']) ? $result['result']['message'] : "");
        }
    }
